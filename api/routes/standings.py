...
def driver_standings(year: int = Query(default=None)):
...
def constructor_standings(year: int = Query(default=None)):
    year = year or current_year()
    rows = query(
        """
        WITH latest_round AS (
        ...
        SELECT
            ...
            '#' || LTRIM(MAX(d.team_color), '#') AS color,
...
def driver_standings_evolution(year: int = Query(default=None)):
    year = year or current_year()
    rows = query(
        """
        SELECT
    ...
    for row in rows:
        ...
        if row["driver"] not in colors:
            colors[row["driver"]] = row["color"]

    return {"data": list(pivoted.values()), "colors": colors}


@router.get("/standings/constructors/evolution")
def constructor_standings_evolution(year: int = Query(default=None)):
    year = year or current_year()
    rows = query(
        """
        SELECT
        ...
        JOIN drivers d ON d.session_id = s.session_id
            AND d.team_name = cs.team_name
        WHERE cs.year = ?
        GROUP BY cs.round_number, cs.event_name, cs.team_name,
                 cs.team_cumulative_points, cs.constructor_championship_position
    ...
    for row in rows:
        ...
        if row["team"] not in colors:
            colors[row["team"]] = row["color"]
